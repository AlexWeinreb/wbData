...
```{r read_gene_coords}
gene_coords <- wb_load_gene_coords(273)
gene_coords
```
...
The downloaded file is stored on the local computer to avoid re-downloading every time. That way, `wbData` can also be used to obtain Wormbase files to use with other software. The cache can be emptied as follows:
